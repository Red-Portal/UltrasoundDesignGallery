...
std::tuple<usdg::LatentGaussianProcess<Kernel>,
		   ...
		   size_t n_init_points)
{
  ...
  auto gp     = usdg::LatentGaussianProcess<Kernel>{
...
generate_datapoint(usdg::Random123& prng,
		   ...
		   usdg::LatentGaussianProcess<Kernel> const& true_gp,
		   size_t n_pseudo,
		   size_t n_dims,
		   double sigma)
{
...
void
ard_prior_predictive_check(usdg::Random123& prng,
			   size_t n_dims,
			   size_t n_pseudo,
			   size_t n_data,
			   ...
			   spdlog::logger* logger)
{
  ...
  auto true_kernel = usdg::Matern52ARD{
...
void
iso_prior_predictive_check(usdg::Random123& prng,
			   size_t n_dims,
			   size_t n_pseudo,
			   size_t n_data,
			   usdg::MvNormal<usdg::DiagonalChol> const& prior_dist,
			   spdlog::logger* logger)
{
  auto hypers  = prior_dist.sample(prng);
  double sigma = exp(hypers[0]);
  auto true_kernel = usdg::Matern52Iso{exp(hypers[1]), exp(hypers[2])};
  ...
  auto make_gram = [&](blaze::DynamicVector<double> const& theta_in)
    ...
    {
      sigma_buf   = exp(theta_in[0]);
      auto kernel = usdg::Matern52Iso{exp(theta_in[1]), exp(theta_in[2])};
      return usdg::compute_gram_matrix(kernel, data_mat);
    };
...

  {
	...
	{
	  auto theta  = blaze::column(theta_samples, k);
	  auto kernel = usdg::Matern52Iso{exp(theta[1]), exp(theta[2])};
	  auto& K_i   = K_samples[k];
	  auto alpha  = usdg::solve(K_i, blaze::column(f_samples, k));
	  auto gp     = usdg::LatentGaussianProcess<usdg::Matern52Iso>{K_i, alpha, kernel};
	  auto [mean, var] = true_gp.predict(true_data, blaze::DynamicVector<double>({i, j}));
	  res += mean;
	}
	z_row.push_back(res / static_cast<double>(n_samples));
...
int main()
{
  ...
  iso_prior_predictive_check(prng, n_dims, n_pseudo, n_data, prior_dist, logger.get());
}
